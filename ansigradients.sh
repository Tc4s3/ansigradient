...
# Ask for gradient size
while true; do
    read -p "Do you want a 16-cell or 8-cell gradient? (16/8): " gradient_size
    case $gradient_size in
        16|8) break;;
...
# Generate gradient based on user's choice
gradient_colors=()
gradient_ansi=()

if [ "$gradient_size" == "16" ]; then
    # 16-cell gradient (8 steps per segment)
    steps=7
    ...
    # 8-cell gradient (3 steps per segment)
    steps=3
    # First half (start to middle)
    for i in $(seq 0 $steps); do
        factor=$(echo "scale=2; $i/$steps" | bc)
